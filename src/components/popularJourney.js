...
const PopularJourney = () => {
  ...
  const CustomArrow = ({ onClick, direction }) => (
    <div className={`${direction}-btn`} onClick={onClick}>
      {direction === "prev" ? (
        <div
          className={`absolute -top-20 left-[59%] sm:left-auto sm:right-20 sm:-top-28 z-10 w-10 sm:w-[54px] h-10 sm:h-[54px] border transition-all flex items-center justify-center rounded-full cursor-pointer m-1 border-[#152B69] hover:bg-[#152B69] hover:text-white`}
        >
          <svg
        ...
        <div
          className={`absolute -top-20 right-[32.5%] sm:right-5 sm:-top-28 top z-10 w-10 sm:w-[54px] h-10 sm:h-[54px] border transition-all flex items-center justify-center rounded-full cursor-pointer m-1 border-[#152B69] hover:text-white hover:bg-[#152B69]`}
        >
          <svg
            className={`w-5 h-5 transition-transform -rotate-90`}
            fill="none"
  ...
  return (
    <section className="flex flex-col items-center justify-center mx-auto p-8 h-[700px] popularJourney">
      <h3 className="font-volkhov text-3xl sm:text-5xl font-medium sm:leading-tight tracking-normal text-left w-full sm:pl-14 text-[#152B69]">
      Popular train <br />journeys to <span className="font-bold">London</span>
      </h3>
        ...
        <Slider {...settings}>
          {destinations.map((item, index) => (
            ...
            >
              ...
              />
              {/* <div className=" absolute top-2  rounded-full right-3 z-30 px-4 py-2 font-medium bg-[#EE2C2C] text-white text-xs">{item.savings}</div> */}
