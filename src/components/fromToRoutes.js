...
const FromToRoutes = () => {
  ...
  const CustomArrow = ({ onClick, direction }) => (
    <div className={`${direction}-btn`} onClick={onClick}>
      {direction === "prev" ? (
        <div
          className={`absolute -top-20 left-[59%] sm:-left-[30px] sm:right-20 sm:top-[40%] z-10 w-10 sm:w-[54px] h-10 sm:h-[54px] border transition-all flex items-center justify-center rounded-full cursor-pointer m-1 border-[#152B69] hover:bg-[#152B69] sm:bg-white hover:text-white`}
        >
          <svg
        ...
        <div
          className={`absolute -top-20 right-[32.5%] sm:-right-[20px] sm:top-[40%] top z-10 w-10 sm:w-[54px] h-10 sm:h-[54px] border transition-all flex items-center justify-center rounded-full cursor-pointer m-1 border-[#152B69] hover:text-white sm:bg-[#152B69] sm:text-white hover:bg-[#152B69]`}
        >
          <svg
            className={`w-5 h-5 transition-transform -rotate-90`}
            fill="none"
  ...
  return (
    <section className="flex flex-col items-center mx-auto sm:my-6 p-8 h-[700px] bg-[#FFF8EC] fromToRoutes">
      <h3 className="font-volkhov text-3xl sm:text-5xl font-medium sm:leading-tight tracking-normal text-left sm:text-center w-full pr-5 sm:pl-10 text-[#152B69]">
        London to Edinburgh Popular <br className="hidden sm:block"/>
        <span className="font-bold">Train Routes</span>
      </h3>
        ...
        <Slider {...settings}>
          {destinations.map((item, index) => (
            ...
            >
              ...
              />
              <div className=" absolute top-2  rounded-full right-3 z-30 px-4 py-2 font-medium bg-[#EE2C2C] text-white text-xs">
                {item.savings}
              </div>
